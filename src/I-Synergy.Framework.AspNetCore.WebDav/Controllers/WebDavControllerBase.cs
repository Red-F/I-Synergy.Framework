...
using ISynergy.Framework.AspNetCore.WebDav.Routing;
...
namespace ISynergy.Framework.AspNetCore.WebDav.Controllers
{
    /// <summary>
    /// The default WebDAV controller
    /// </summary>
    public abstract class WebDavControllerBase : ControllerBase
    {
        ...
        protected WebDavControllerBase(IWebDavContext context, IWebDavDispatcher dispatcher, ILogger<WebDavIndirectResult> responseLogger = null)
        {
            _context = context;
        ...
        {
            var result = await _dispatcher.Class1.PutAsync(path ?? string.Empty, HttpContext.Request.Body, cancellationToken);
            return new WebDavIndirectResult(_dispatcher, result, _responseLogger);
        }
        ...
        public async Task<IActionResult> CopyAsync(
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            var result = await _dispatcher.Class1.CopyAsync(path ?? string.Empty, new Uri(destination, UriKind.RelativeOrAbsolute), cancellationToken);
            return new WebDavIndirectResult(_dispatcher, result, _responseLogger);
        }
        ...
        public async Task<IActionResult> MoveAsync(
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            var result = await _dispatcher.Class1.MoveAsync(path ?? string.Empty, new Uri(destination, UriKind.RelativeOrAbsolute), cancellationToken);
            return new WebDavIndirectResult(_dispatcher, result, _responseLogger);
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (lockinfo == null)
            {
                ...
                result = await _dispatcher.Class2.RefreshLockAsync(path ?? string.Empty, ifHeader, timeoutHeader, cancellationToken);
            }
            else
            {
                // Lock
                result = await _dispatcher.Class2.LockAsync(path ?? string.Empty, lockinfo, cancellationToken);
            }
