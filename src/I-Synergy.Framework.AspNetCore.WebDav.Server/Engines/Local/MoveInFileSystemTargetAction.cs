...
namespace ISynergy.Framework.AspNetCore.WebDav.Server.Engines.Local
{
    ...
    public class MoveInFileSystemTargetAction : ITargetActions<CollectionTarget, DocumentTarget, MissingTarget>
    {
        ...
        {
            var doc = await source.MoveToAsync(destination.Parent.Collection, destination.Name, cancellationToken);
        ...
        public async Task<ActionResult> ExecuteAsync(IDocument source, DocumentTarget destination, CancellationToken cancellationToken)
        {
            try
            {
                ...
                await source.MoveToAsync(destination.Parent.Collection, destination.Name, cancellationToken);
