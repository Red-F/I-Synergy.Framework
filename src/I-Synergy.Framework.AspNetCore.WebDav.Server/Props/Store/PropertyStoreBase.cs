...
namespace ISynergy.Framework.AspNetCore.WebDav.Server.Props.Store
{
    ...
    public abstract class PropertyStoreBase : IPropertyStore
    {
        ...
        {
            var elements = await GetAsync(entry, cancellationToken);
        ...
        {
            return (await RemoveAsync(entry, new[] { name }, cancellationToken)).Single();
        }
        ...
        public virtual async Task RemoveAsync(IEntry entry, CancellationToken cancellationToken)
        {
            ...
            if (elements.Count != names.Count)
            {
                ...
                await UpdateETagAsync(entry, cancellationToken);
            }

            await RemoveAsync(entry, names, cancellationToken);
        }
        ...
        {
            var element = await GetAsync(entry, name, cancellationToken);
            if (element == null)
