...
namespace ISynergy.Framework.AspNetCore.WebDav.Server.Locking
{
    ...
    public abstract class LockManagerBase : ILockManager
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                var newLock = await LockAsync(lockRequirements, cancellationToken);
        ...
        public async Task<LockRefreshResult> RefreshLockAsync(IFileSystem rootFileSystem, IfHeader ifHeader, TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            foreach (var ifHeaderList in ifHeader.Lists.Where(x => x.RequiresStateToken))
            {
                ...
                if (pathInfo.EntityTag == null)
                {
                    ...
                    {
                        var selectionResult = await rootFileSystem.SelectAsync(ifHeaderList.Path.OriginalString, cancellationToken);
                        ...
                        pathInfo.EntityTag = await selectionResult.TargetEntry.GetEntityTagAsync(cancellationToken);
                    }
                }
            }
            ...
            using (var transaction = await BeginTransactionAsync(cancellationToken))
            {
                foreach (var ifHeaderList in ifHeader.Lists.Where(x => x.RequiresStateToken))
                {
                    ...
                    if (pathInfo.ActiveLocks == null)
                    {
                        ...
                        var entryLocks = (from l in await transaction.GetActiveLocksAsync(cancellationToken)
                ...
                {
                    await transaction.UpdateAsync(newLock, cancellationToken);
                }
        ...
        public async Task<LockReleaseStatus> ReleaseAsync(string path, Uri stateToken, CancellationToken cancellationToken)
        {
            ...
            using (var transaction = await BeginTransactionAsync(cancellationToken))
            {
                activeLock = await transaction.GetAsync(stateToken.OriginalString, cancellationToken);
                if (activeLock == null)
                {
                    if (_logger.IsEnabled(LogLevel.Information))
                ...
                await transaction.RemoveAsync(stateToken.OriginalString, cancellationToken);
                await transaction.CommitAsync(cancellationToken);
            }
                ...
                if (pathInfo.EntityTag == null)
                {
                    if (ifHeaderList.RequiresEntityTag)
                    {
                        var selectionResult = await rootFileSystem
                            .SelectAsync(ifHeaderList.Path.OriginalString, cancellationToken);
                        if (selectionResult.IsMissing)
                        {
                            // Probably locked entry not found
                            continue;
