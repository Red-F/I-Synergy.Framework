    ...
    public class PropPatchHandler : IPropPatchHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    while (await propEnum.MoveNext(cancellationToken))
                    {
                ...
                var changes =
                    await ApplyChangesAsync(targetEntry, properties, request, cancellationToken);
                ...
                else
                {
                    ...
                    if (targetPropStore != null)
                        await targetPropStore.UpdateETagAsync(targetEntry, cancellationToken);
            ...
            foreach (var element in remove.prop.Any)
            {
                ...
                if (property != null)
                {
                    ...
                    else
                    {
                        try
                        {
                            var oldValue = await property.GetXmlValueAsync(cancellationToken);
                            var success = await entry.FileSystem.PropertyStore.RemoveAsync(entry, propertyKey, cancellationToken);
