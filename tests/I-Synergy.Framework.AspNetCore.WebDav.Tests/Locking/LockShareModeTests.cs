        ...
        where T : class, ILockServices
    {
        ...
        public async Task TestTwoSharedAsync()
        {
            ...
            var testLock = new Lock(
                ...
                LockShareMode.Shared,
                TimeSpan.FromMinutes(1));
            var result1 = await lockManager.LockAsync(testLock, ct);
            ValidateLockResult(result1);
            var result2 = await lockManager.LockAsync(testLock, ct);
        ...
        public async Task TestExclusiveRootUnlockWithSharedSubAsync()
        {
            ...
            ValidateLockResult(result1);
            var resultRelease1 = await lockManager.ReleaseAsync(result1.Lock.Path, new Uri(result1.Lock.StateToken), ct);
