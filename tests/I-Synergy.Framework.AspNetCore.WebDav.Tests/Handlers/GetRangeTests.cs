...
namespace ISynergy.Framework.AspNetCore.WebDav.Tests.Handlers
{
    public class GetRangeTests : ServerTestsBase
    {
        ...
        public async Task GetWithoutRangeTest()
        {
            ...
            {
                using (var response = await client.GetAsync("test1.txt", ct))
                {
        ...
        public async Task GetWithTwoRangesTest()
        {
            ...
            using (var client = Server.CreateClient())
            {
                ...
                using (var response = await client.SendAsync(request, ct))
                {
                    ...
                    var multipart = await ReadMultipartAsync(content, ct);
        ...
        private async Task FillAsync(IDocument document, int maxLength, CancellationToken ct)
        {
            ...
            {
                await stream.WriteAsync(_testBlock.Value, 0, Math.Min(_testBlock.Value.Length, maxLength), ct);
            }
        }
    }
}
